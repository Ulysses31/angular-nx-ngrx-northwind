...
TableProcessor.prototype.beginTable = function (writer) {
  ...
  availableWidth =
    writer.context().availableWidth - this.offsets.total;
  ...
  this.tableWidth =
    tableNode._offsets.total + getTableInnerContentWidth();
  ...
  function prepareRowSpanData() {
    ...
    for (
      var i = 0, l = self.tableNode.table.body[0].length;
      i < l;
      i++
    ) {
      var paddings =
        self.layout.paddingLeft(i, self.tableNode) +
        self.layout.paddingRight(i, self.tableNode);
...
TableProcessor.prototype.onRowBreak = function (rowIndex, writer) {
  ...
  return function () {
    var offset =
      self.rowPaddingTop + (!self.headerRows ? self.topLineWidth : 0);
...
TableProcessor.prototype.beginRow = function (rowIndex, writer) {
  this.topLineWidth = this.layout.hLineWidth(
    rowIndex,
    this.tableNode
  );
  this.rowPaddingTop = this.layout.paddingTop(
    rowIndex,
    this.tableNode
  );
...
TableProcessor.prototype.drawHorizontalLine = function (
  ...
  if (lineWidth) {
    ...
    for (var i = 0, l = this.rowSpanData.length; i < l; i++) {
      ...
      if (borderColor == null) {
        borderColor = isFunction(this.layout.hLineColor)
...
TableProcessor.prototype.drawVerticalLine = function (
  x,
  y0,
  y1,
  vLineColIndex,
  ...
  if (
    borderColor == null &&
    cellBefore &&
    cellBefore._rowSpanCurrentOffset
  ) {
    var rowCellBeforeAbove =
  ...
  if (
    ...
    var rowCurrentCellAbove =
      body[vLineRowIndex - currentCell._rowSpanCurrentOffset][
        vLineColIndex
      ];
  ...
  if (borderColor == null) {
    borderColor = isFunction(this.layout.vLineColor)
...
TableProcessor.prototype.endRow = function (
  ...
  for (
    var yi = skipOrphanePadding ? 1 : 0, yl = ys.length;
    yi < yl;
    yi++
  ) {
    ...
    for (i = 0, l = xs.length; i < l; i++) {
      ...
      if (i < l - 1) {
        ...
        if (!fillColor) {
          fillColor = isFunction(this.layout.fillColor)
        ...
        if (!isNumber(fillOpacity)) {
          fillOpacity = isFunction(this.layout.fillOpacity)
        ...
        if (fillColor) {
          ...
          if (
            ...
            widthRightBorder =
              this.layout.vLineWidth(colIndex + 1, this.tableNode) /
              2;
          } else {
            widthRightBorder = 0;
          }

          var x1f = this.dontBreakRows
    ...
    if (
      ...
      this.layout.hLineWhenBroken !== false
    ) {
  ...
  for (i = 0, l = row.length; i < l; i++) {
    if (row[i].rowSpan) {
      ...
      if (row[i].rowSpan && row[i].rowSpan > 1) {
        for (var j = 1; j < row[i].rowSpan; j++) {
          this.tableNode.table.body[rowIndex + j][
            i
          ]._rowSpanCurrentOffset = j;
        }
      }
    }
  ...
  function getLineXs() {
    ...
    for (
      var i = 0, l = self.tableNode.table.body[rowIndex].length;
      i < l;
      i++
    ) {
      if (!cols) {
