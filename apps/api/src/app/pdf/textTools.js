...
TextTools.prototype.buildInlines = function (
  ...
  function getTrimmedWidth(item) {
    return Math.max(
      0,
      item.width - item.leadingCut - item.trailingCut
    );
  }
};
 ...
 */
TextTools.prototype.sizeOfString = function (
  text,
  styleContextStack
) {
...
function normalizeTextArray(array, styleContextStack) {
  ...
  for (var i = 0, l = array.length; i < l; i++) {
    ...
    if (lastWord && words.length) {
      ...
      var wrapWords = splitWords(
        normalizeString(lastWord + firstWord),
        false
      );
