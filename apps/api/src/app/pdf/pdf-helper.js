const pdfMakePrinter = require('./printer');
const path = require('path');

export const addRow = (cols) => {
  const row = {
    alignment: 'justify',
    columns: []
  };
  ...
  cols.forEach((col) =>
    row.columns.push({
      ...
      table: {
        body: [
          [{ text: col.title, style: this.addTitleStyle(col.title) }],
          [{ text: col.value, style: this.addBodyStyle(col.title) }]
        ]
      },
      layout: 'noBorders'
    })
...
export const addTitleStyle = (column) => {
  if (column === 'A.A. αίτησης') {
    return 'bodyRightTitle';
  } else {
    return 'bodyTitle';
  }
};

export const addBodyStyle = (column) => {
  if (column === 'A.A. αίτησης') {
    return 'bodyRight';
  } else if (column === 'Τύπος αίτησης') {
    return 'bodyLeft';
  } else {
    return 'body';
  }
...
export const addTableData = (headers, model) => {
  let widthList = [];
  let tmpRow = [];
  const tmpMasterRow = [];
  let vCnt = 0;
  ...
  model.forEach((item) => {
    ...
    headers.forEach((header) => {
      ...
      tmpRow.push(tmpModel);
    });
    tmpMasterRow.push(tmpRow);
    data.table.body.push(tmpRow);
  });

  return data;
};

export const createPdfBinary = (pdfDoc, callback) => {
  var fontDescriptors = {
    Roboto: {
      normal: path.join(
        __dirname.replace('dist\\apps\\api', 'apps\\api\\src\\app\\'),
        'pdf/fonts/roboto/Roboto-Regular.ttf'
      ),
      bold: path.join(
        __dirname.replace('dist\\apps\\api', 'apps\\api\\src\\app\\'),
        'pdf/fonts/roboto/Roboto-Medium.ttf'
      ),
      italics: path.join(
        __dirname.replace('dist\\apps\\api', 'apps\\api\\src\\app\\'),
        'pdf/fonts/roboto/Roboto-Italic.ttf'
      ),
      bolditalics: path.join(
        __dirname.replace('dist\\apps\\api', 'apps\\api\\src\\app\\'),
        'pdf/fonts/roboto/Roboto-MediumItalic.ttf'
      )
    }
  };
...
//     },
//     {
//       text: 'handling fill color opacity...',
//       margin: [0, 20, 0, 8]
//     },
//     {
