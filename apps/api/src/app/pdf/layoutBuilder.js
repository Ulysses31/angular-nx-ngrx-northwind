    ...
    for (var index = 0; index < linearNodeList.length; index++) {
      ...
      if (node.pageBreak !== 'before' && !node.pageBreakCalculated) {
        ...
        if (pageBreakBeforeFct.length > 1) {
          for (
            var ii = index + 1, l = linearNodeList.length;
            ii < l;
            ii++
          ) {
            if (
...
LayoutBuilder.prototype.addBackground = function (background) {
  ...
  if (pageBackground) {
    ...
    pageBackground =
      this.docPreprocessor.preprocessDocument(pageBackground);
    this.processNode(this.docMeasure.measureDocument(pageBackground));
    this.writer.commitUnbreakableBlock(0, 0);
    context.backgroundLength[context.page] +=
      pageBackground.positions.length;
  }
};
...
LayoutBuilder.prototype.addDynamicRepeatable = function (
  ...
  for (
    var pageIndex = 0, l = pages.length;
    pageIndex < l;
    pageIndex++
  ) {
...
LayoutBuilder.prototype.addWatermark = function (
  ...
  watermark.opacity = isNumber(watermark.opacity)
    ? watermark.opacity
    : 0.6;
  ...
  function getWatermarkSize(watermark, fontProvider) {
    ...
    var size = textTools.sizeOfString(
      watermark.text,
      styleContextStack
    );
...
LayoutBuilder.prototype.processNode = function (node) {
  ...
  applyMargins(function () {
    ...
    if (absPosition) {
      ...
      self.writer
        .context()
        .moveTo(absPosition.x || 0, absPosition.y || 0);
    }
  ...
  if (line && line.inlines && isArray(line.inlines)) {
    for (var i = 0, l = line.inlines.length; i < l; i++) {
      ...
      if (line.inlines[i]._pageRef) {
        line.inlines[i]._pageNodeRef =
          line.inlines[i]._pageRef._nodeRef;
      }
    }
  }
  ...
  while (
    ...
    if (
      ...
      var maxChars = Math.floor(
        line.getAvailableWidth() / widthPerChar
      );
      if (maxChars < 1) {
        maxChars = 1;
      }
