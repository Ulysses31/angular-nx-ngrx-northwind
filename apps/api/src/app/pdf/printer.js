...
var getSvgToPDF = function () {
  try {
    ...
    throw new Error(
      'Please install svg-to-pdfkit to enable svg nodes'
    );
  }
};
...
function calculatePageHeight(pages, margins) {
  function getItemHeight(item) {
    if (isFunction(item.item.getHeight)) {
      ...
      return item.item.y1 > item.item.y2
        ? item.item.y1
        : item.item.y2;
    } else {
...
function renderVector(vector, pdfKitDoc) {
  ...
  var fillOpacity = isNumber(vector.fillOpacity)
    ? vector.fillOpacity
    : 1;
  var strokeOpacity = isNumber(vector.strokeOpacity)
