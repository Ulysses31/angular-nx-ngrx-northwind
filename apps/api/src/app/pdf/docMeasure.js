...
DocMeasure.prototype.measureNode = function (node) {
  ...
  function getNodeMargin() {
    ...
    if (node.style) {
      var styleArray = isArray(node.style)
        ? node.style
        : [node.style];
...
DocMeasure.prototype.measureImageWithDimensions = function (
  node,
  dimensions
) {
  if (node.fit) {
    ...
    node._height =
      node.height ||
      (dimensions.height * node._width) / dimensions.width;

    if (isNumber(node.maxWidth) && node.maxWidth < node._width) {
      node._width = node._minWidth = node._maxWidth = node.maxWidth;
      node._height =
        (node._width * dimensions.height) / dimensions.width;
    }
...
DocMeasure.prototype.measureToc = function (node) {
  ...
  if (node.toc._items.length > 0) {
    ...
    for (var i = 0, l = node.toc._items.length; i < l; i++) {
      ...
      var lineNumberStyle =
        item._textNodeRef.tocNumberStyle || numberStyle;
...
DocMeasure.prototype.buildOrderedMarker = function (
  counter,
  styleStack,
  type,
  separator
...
DocMeasure.prototype.measureOrderedList = function (node) {
  ...
  for (var i = 0, l = items.length; i < l; i++) {
    ...
    if (!item.ol && !item.ul) {
      var counterValue = isNumber(item.counter)
        ? item.counter
        : counter;
  ...
  function measureCb(_this, data) {
    return function () {
      if (isObject(data)) {
        ...
        data.fillOpacity =
          _this.styleStack.getProperty('fillOpacity');
      }
