...
const router = Router();
const pdfMakePrinter = require('../pdf/printer');
...
router.post('/', async (req, res) => {
  ...
  var docDefinition = {
    content: [
      ...
      addTableData(result.pdfHeaders, result.pdfModel)
    ],
    styles: {
      header: {
        fontSize: 15,
  ...
  createPdfBinary(docDefinition, (response) => res.send(response));
});

const addTableData = (headers, model) => {
  let widthList = [];
  let tmpRow = [];
  const tmpMasterRow = [];
